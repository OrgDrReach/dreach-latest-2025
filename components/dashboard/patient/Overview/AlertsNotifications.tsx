...
const AlertsNotifications: React.FC<AlertsNotificationsProps> = ({
  ...
  const getAlertIcon = (type: Alert["type"]) => {
    switch (type) {
      case "medication":
        return <FaPills className="w-5 h-5 text-blue-200" />;
      case "appointment":
  ...
  const getAlertColor = (type: Alert["type"]) => {
    switch (type) {
      case "medication":
        return "bg-blue-400 text-blue-900 border-blue-200";
      case "appointment":
      ...
      <CardContent className="pt-6">
        <ScrollArea className="h-[300px] pr-4">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className="mb-4 p-3 bg-white dark:bg-gray-700 rounded-lg shadow"
            >
