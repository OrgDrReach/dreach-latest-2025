...
const Book = ({ onClick }: { onClick?: () => void }) => {
	...
	// Modified useEffect to only run once when onClick is first provided
	useEffect(() => {
		if (onClick && !isOpen) {
			setIsOpen(true);
		}
	}, [onClick]); // Remove isOpen from dependency array
	...
	return (
		<>
			...
			{isOpen && (
				<div className="fixed z-10 inset-0 backdrop-blur-sm bg-black/30 overflow-y-auto h-full w-full flex justify-center items-start md:items-center pt-10 md:pt-0">
					<div className="relative bg-white/90 backdrop-filter rounded-lg shadow-lg p-6 mx-4 my-10 md:max-w-lg md:mx-auto">
